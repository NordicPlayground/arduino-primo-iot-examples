...
/** @file
 *
 * @defgroup iot_ipv6_coap_server_eddystone_app main.c
 * @{
 * @brief Commissioning using Eddystone beacons example.
 *
...
eui64_t                       eui64_local_iid;                                                /**< Local EUI64 value that is used as the IID for the device. */
...
static const char             m_led2_name[]   = "led2";
...
static const char m_default_url_local[] = {'n', 'o', 'd', 'e', '.', 'c', 'f', 'g'};
static const char m_default_url[]       = {'g', 'o', 'o', '.', 'g', 'l', '/', 'P', 'a', 'E', 'Y', 'F', '1'}; 
...
/**@brief Function for handling CoAP periodical time ticks.
*/
...
static void buttons_init(void)
{
    ...
    #define EDDY_URL_MODE_BUTTON_ID 0
#endif // EDDYSTONE_ENABLED
...
static void led_value_get(uint32_t led_mask, coap_content_type_t content_type, char ** str)
{
    ...
    {
        static char response_str_led2_true[15] = {"{\"led2\": True}\0"};
        static char response_str_led2_false[16] = {"{\"led2\": False}\0"};

        if((bool)LED_IS_ON(led_mask) == true)
        {
            *str = response_str_led2_true;
        }
        else
        {
            *str = response_str_led2_false;
        }
    }
...
static void led2_callback(coap_resource_t * p_resource, coap_message_t * p_request)
{
    handle_led_request(LED_TWO, p_request, p_resource);
}
...
static void coap_endpoints_init(void)
{
    ...
    static coap_resource_t led2;
    err_code = coap_resource_create(&led2, m_led2_name);
    APP_ERROR_CHECK(err_code);

    led2.permission      = (COAP_PERM_GET | COAP_PERM_PUT);
    led2.callback        = led2_callback;
    led2.ct_support_mask = COAP_CT_MASK_APP_JSON | COAP_CT_MASK_PLAIN_TEXT;

    err_code = coap_resource_child_add(&lights, &led2);
    APP_ERROR_CHECK(err_code);
...
#ifdef EDDYSTONE_ENABLED
static void eddystone_update(bool enable_local_url)
{
    eddystone_url_settings_t eddystone_url_settings;
    uint32_t err_code;
    
    if (enable_local_url == true)
    {
        eddystone_url_settings.rssi   = -18;
        eddystone_url_settings.prefix = APP_EDDYSTONE_PREFIX_HTTPS;
        eddystone_url_settings.p_url  = (char*)m_default_url_local;
        eddystone_url_settings.size   = sizeof(m_default_url_local);      
    }
    else
    {
        eddystone_url_settings.rssi   = -18;
        eddystone_url_settings.prefix = APP_EDDYSTONE_PREFIX_HTTPS;
        eddystone_url_settings.p_url  = (char*)m_default_url;
        eddystone_url_settings.size   = sizeof(m_default_url);      
    }
    err_code = app_eddystone_update_url(&eddystone_url_settings);
    APP_ERROR_CHECK(err_code);
}
#endif // EDDYSTONE_ENABLED
...
int main(void)
{
    ...
    buttons_init();
#ifdef EDDYSTONE_ENABLED
    bool enable_local_url;
    err_code = app_button_is_pushed(EDDY_URL_MODE_BUTTON_ID,&enable_local_url);
    APP_ERROR_CHECK(err_code);
#endif // EDDYSTONE_ENABLED
#endif // COMMISSIONING_ENABLED
    ...
    // Hardcode EUI_48.
    uint8_t mac[EUI_48_SIZE] = {0xB4, 0xD1, 0x82, 0x67, 0x1C, 0x00};
...
#ifdef COMMISSIONING_ENABLED
#ifdef EDDYSTONE_ENABLED
    eddystone_update(enable_local_url);
#endif // EDDYSTONE_ENABLED 
#endif // COMMISSIONING_ENABLED
