...
#ifdef EDDYSTONE_ENABLED
static bool                               m_eddystone_enabled = false;                              /**< Variable used to toggle between advertising node configuration and Eddystone beacons. */
#endif // EDDYSTONE_ENABLED                                              
...
#ifdef EDDYSTONE_ENABLED
            if (m_eddystone_enabled == true)
            {
                (void)app_eddystone_adv_params_get(pp_node_adv_params);
            }
#endif // EDDYSTONE_ENABLED 
...
#ifdef EDDYSTONE_ENABLED
/**@brief Function for toggling advertisement of Eddystone beacons.
 *
 * @param[in]    wall_clock_value    Wall clock value from the IoT Timer module.
 */
static inline void eddystone_time_tick(iot_timer_time_in_ms_t wall_clock_value)
{
    // Send event to medium.
    if (m_current_mode == NODE_MODE_CONFIG)
    {
        ...
        {
            m_eddystone_enabled = !m_eddystone_enabled; // Toggle
...
{
#ifdef EDDYSTONE_ENABLED
    eddystone_time_tick(wall_clock_value);
#endif // EDDYSTONE_ENABLED
...
#ifdef EDDYSTONE_ENABLED   
    err_code = app_eddystone_init(NULL);
#endif // EDDYSTONE_ENABLED
